...
void DataDependence::getDataDependencies(Function &F, MemoryDependenceResults &MDA, AliasAnalysis &AA) {
  ...
  for (inst_iterator i = inst_begin(F); i != inst_end(F); ++i) {
    ...
    processDepResult(F, inst, MDA, AA);

  } // end for (inst_iterator)
}

void DataDependence::processDepResult(Function &FI, Instruction *inst, MemoryDependenceResults &MDA, AliasAnalysis &AA) {
  // TODO: This is probably a good place to check of the dependency information is calculated on-demand
    //FunctionAnalysisManager FAM;
    //MemoryDependenceResults MDResults = MDA.run(FI, FAM);
  ...
  if (!Res.isNonLocal()) {
    ...
    if (LoadInst *LI = dyn_cast<LoadInst>(inst)) {
      ...
      //llvm_unreachable("Unknown memory instruction!");
        errs() <<"Unknown memory instruction! "<<inst->getName()<<"\n";
    }
