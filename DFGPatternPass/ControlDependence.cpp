...
void ControlDependence::toDot(std::string name) const {
 ...
 // std::ofstream out;
  ...
  std::string errInfo; std::error_code EC; sys::fs::OpenFlags flags;
  raw_fd_ostream out(name, EC, flags);
  ...
  for (auto i = controlDeps_.begin(), ei = controlDeps_.end(); i != ei; ++i) {
    BasicBlock *tail;
    tail = i->first;
...
void ControlDependence::insertDotNode(raw_fd_ostream &out, BasicBlock *node) const {
  ...
  bbo << (*node).getName();
  std::replace( bbo.str().begin(), bbo.str().end(), '"', ' ');
...
void ControlDependence::replaceAll(std::string& str, const std::string& from, const std::string& to) {
    if(from.empty())
        return;
    size_t start_pos = 0;
